...
def run_merger():
    ...
    # --------------------------------------------------------
    # NEW: Count RawPrices AFTER merge + delta
    # --------------------------------------------------------
    cursor.execute("SELECT COUNT(*) FROM tblRawPrices;")
    raw_after = cursor.fetchone()[0]

    log(f"RawPrices AFTER merge: {raw_after:,}")

    delta = raw_after - raw_before
    log(f"Rows added in this merge: {delta:,}")

    # --------------------------------------------------------
    # 8. FINAL DATABASE SUMMARY
    # --------------------------------------------------------
    log(f"Total rows in RawPrices after merge: {raw_after:,}")
